...
import SubscriberDetailTabs from '@/components/admin/subscriber-detail-tabs'
import SubscriptionHistoryPanel from '@/components/admin/subscription-history-panel'
import SubscriptionOrderPanel from '@/components/admin/subscription-order-panel'
...
export default async function AdminSubscriberDetailPage({ params }: PageProps) {
  ...
  const [screenshotUrl, orders, historyOrders, linkableApplications] = await Promise.all([
    getPaymentScreenshotUrl(application.payment_screenshot_path),
    isFulfillmentStatus(application.status)
      ? listOrdersForApplication(application.id)
      : Promise.resolve([]),
    isFulfillmentStatus(application.status)
      ? listHistoryOrders(application)
      : Promise.resolve([]),
    isFulfillmentStatus(application.status)
      ? listLinkableApplications(application)
      : Promise.resolve([]),
  ])

  const showOrderTab = isFulfillmentStatus(application.status)
  const showHistoryTab =
    isFulfillmentStatus(application.status) &&
    (historyOrders.length > 0 ||
      linkableApplications.length > 0 ||
      Boolean(application.previous_application_id))
      ...
      <section className="mb-8 border border-brand-earth/60 bg-brand-void/40 p-6">
        ...
        <dl className="grid gap-5 sm:grid-cols-2">
          <DetailField
            ...
            multiline
          />
          <DetailField label="Books read (signup)" value={application.books_read} multiline />
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,20rem)]">
        <section className="border border-brand-earth/60 bg-brand-void/40 p-6">
          ...
          <dl className="grid gap-5 sm:grid-cols-2">
            ...
            <DetailField label="Duration" value={formatPlanDuration(application.plan_id)} />
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
      ...
      <SubscriberDetailTabs
        showOrderTab={showOrderTab}
        showHistoryTab={showHistoryTab}
        orderPanel={
          <SubscriptionOrderPanel planId={application.plan_id} orders={orders} />
        }
        historyPanel={
